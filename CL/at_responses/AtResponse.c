...
AtResponse* AtResponse_create(ResponseStatus status, StringArray response, Param* wanted, int wanted_size) {
    ...
    for (int i = 0; i < response.size; i++) {
        at_response->rows_array.values[i].text = calloc(MAX_RESPONSE_ROW_SIZE, sizeof(char));
        at_response->rows_array.values[i].length = response.values[i].length;//or MAX_RESPONSE_ROW_SIZE
    ...
    for (int i = 0; i < wanted_size; i++) {
        at_response->wanted_params_array[i].name = malloc(MAX_RESPONSE_ROW_SIZE * sizeof(char));
        strcpy(at_response->wanted_params_array[i].name, wanted[i].name);
        at_response->wanted_params_array[i].value = malloc(MAX_RESPONSE_ROW_SIZE * sizeof(char));
